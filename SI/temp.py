...
>>>>>>> 2c57bd60053d53450ae1157369c78c12ecf1e0f7:main.py
...
<<<<<<< HEAD:SI/temp.py
def train(args, train_dataset, y_true, task, all_tasks, model, task_num, tokenizer, accuracy_matrix, consolidate_fisher,
          consolidate_mean):
=======
...
<<<<<<< HEAD:SI/temp.py
    ...
    if config['optimizer'] in ['SGD', 'RMSprop']:
        optimizer_arg['momentum'] = config['momentum']
    elif config['optimizer'] in ['Rprop']:
        optimizer_arg.pop('weight_decay')
    elif config['optimizer'] == 'amsgrad':
        optimizer_arg['amsgrad'] = True
        config['optimizer'] = 'Adam'

    optimizer = torch.optim.__dict__[config['optimizer']](**optimizer_arg)
...
>>>>>>> 2c57bd60053d53450ae1157369c78c12ecf1e0f7:main.py
    ...
    if reset_optimizer:  # Reset optimizer before learning each task
        logger.info('Optimizer is reset!')
        # init_optimizer()
        # resetting optimizer
        optimizer = torch.optim.__dict__[config['optimizer']](**optimizer_arg)
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=config['schedule'],
                                                         gamma=0.1)
    # logger.info('#reg_term:', len(regularization_terms))
...
<<<<<<< HEAD:SI/temp.py
    ...
    # For all models transfer them to device and save the models
=======
        if args.n_gpu<=1:
            if args.mas:
...
<<<<<<< HEAD:SI/temp.py
        train_dataset, y_true = load_and_cache_examples(args, tasks[i], tokenizer, evaluate=False)
        print(len(train_dataset), len(y_true))
        # print(len(train_dataset[0:100]))
        # call the train fuction for each task
        global_step, tr_loss, accuracy_matrix, model = train(new_args, train_dataset, y_true, tasks[i], tasks,
                                                             models[i][1], i, tokenizer, accuracy_matrix,
                                                             consolidate_fisher, consolidate_mean)
=======
